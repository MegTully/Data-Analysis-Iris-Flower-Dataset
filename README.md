...
When first carrying out the above calculations for correlation coefficients I forgot that I needed to group by species and I printed a table of correlation coefficients for each variable not including species and I noticed alot of negative correlation coefficients. I was confused at first but then found it very interesting that even though when I calculated the correlation coefficient for each pair of variables within thier species group there was no negative relationships but when you look at the dataset as a whole and analyse the sepals and petals properties in general there are negative influences in some cases.
...
After researching the factors that affect the correlation [21] I came to the conclusion the reason the correlation may have changed from positive to negative when analysing the dataset overall vs grouping by species could be because of the difference in some distribution shapes as some are skewed distributions and some are normal distributions. It could also be due to outliers in the dataset but I don't think this is the reason as there are not that many outliers. I think the most viable reason is because of the variability in the dataset because the measurements for each type of flower are within different scale ranges. For example if you have three clusters of data placed in the plot in a position that appears to be a declining slope but within the clusters when you zoom in and spread them out the data displays an increasing slope. After discovering this I believe it is more accurate to analyse the data grouped by type or else normalise the scales so that the scale of measurements recorded don't affect the results.

## Normalising The Data
I decided to try and normalise the data and then get the correlation coefficients to see if they changed[25],[26],[27]. The formula used to normailise the values in each row subtracts the minimum value from each column and then divides the result by the difference between the maximum and minimum value[25]. First I imported the sklearn package so I could use the preprocessing function in it. In order to normalise the data I had to normalise each column separately so I used the numpy package to put each column into an array and then used the "preprocessing.normalise" function to normalise all the data entries. All the data entries representing the recorded measurements now were convereted to values on a scales between -1 and 1.Then I got the correlation coefficient of each pair of columns/variables as I did previously. Unfortuneately I still got the same correlation coefficients as I did before I normalised the data. I attempted to try another method of normalising the data called the MaxMin scaler which scales each data entry individually using the standard deviation,maximum and minimum values but it did not work out for me [28][29]. 
...
[25]https://www.journaldev.com/45109/normalize-data-in-python
[26]https://numpy.org/doc/stable/reference/generated/numpy.corrcoef.html
[27]https://stackoverflow.com/questions/21764475/python-scaling-numbers-column-by-column-with-pandas
[28]https://stackoverflow.com/questions/43834242/sklearn-minmaxscaler-scale-specific-columns-only
[29]https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.MinMaxScaler.html